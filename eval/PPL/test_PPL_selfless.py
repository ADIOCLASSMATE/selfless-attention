...
from models.modeling_model.modeling_selfless import Qwen3ForCausalLM as SelflessLM
from utils.diffusion_utils import DiffusionLanguage
from utils.utils import get_AR_attention_mask, get_diffusion_attention_mask, get_full_attention_mask, get_selfless_mask, get_selfless_ar_mask
...
if __name__ == "__main__":
    ...
    model_path_dict = {
        "omega": "output_final/omega-preload/hf_model-final",
    }
    
    dataset_list = ["NeelNanda/pile-10k", "wikitext-2-raw-v1", "openwebtext",]
    ...
    for model_name, model_path in model_path_dict.items():
        ...
        elif "selfless" in model_name.lower() or "omega" in model_name.lower():
            print(f"selfless!")
            modelclass = SelflessLM
        else:
            continue
            ...
            for step, batch in tqdm(enumerate(dataloader), total=len(dataloader), disable=not accelerator.is_main_process):
                with torch.no_grad():
                    ...
                    if "mad" in model_name.lower():
                        ...
                        loss_diff = unwrapped_model.forward_process_inference(
                        ...
                        AR_attention_mask = get_selfless_ar_mask(seq_len=L, device=accelerator.device)
                        loss_ar = unwrapped_model.forward_process(
