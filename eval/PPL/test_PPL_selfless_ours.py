...
if __name__ == "__main__":
    ...
    model_path_dict = {
        "omega-80BT-ours": "output_final/omega-80BT/hf_model-final",
    }
    ...
    for model_name, model_path in model_path_dict.items():
        if "selfless" in model_name.lower() or "omega" in model_name.lower():
        ...
        for dataset_name in dataset_list:
            ...
            for step, batch in tqdm(enumerate(dataloader), total=len(dataloader), disable=not accelerator.is_main_process):
                with torch.no_grad():
                    ...
                    # --- Diffusion Loss ---
                    if "selfless" in model_name.lower() or "omega" in model_name.lower():
                        diffusion_attention_mask = get_selfless_mask(v_sample=v_sample, seq_len=text_ids.size(-1), device=accelerator.device)
                        ...
                        AR_attention_mask = get_selfless_ar_mask(seq_len=text_ids.size(-1), device=accelerator.device)
