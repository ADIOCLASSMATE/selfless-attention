...
用法（仓库根目录，单卡）：
  ...
  # 可选：--max_len 1024 覆盖 yaml 里的 rolling window 长度
...
def main():
    ...
    ap.add_argument("--seed", type=int, default=42, help="native Monte Carlo 估计随机种子")
    args = ap.parse_args()
    ...
    mc_num = int(cfg.get("mc_num", 1) or 1)
    max_len = int(cfg.get("max_len", 2048) or 2048)
    print(f"[text_likelihood] project={project} | family={family} | "
          f"mc_num={mc_num} | max_len={max_len} | seed={args.seed}")

    # 实例化对应 worker（它内部用 accelerate.Accelerator() 单进程即可，无需 accelerate launch）
    worker_mod = load_worker_module(family)
    worker_mod.set_seed(args.seed)
    worker = worker_mod.DLLMEvalHarness(config_path=args.config)
    if args.max_len is not None:
        worker.eval_config.max_len = args.max_len
