...
from utils.utils import get_AR_attention_mask, get_full_attention_mask
...
class Qwen3Attention(nn.Module):
    ...
    def forward(
        ...
        **kwargs: Unpack[FlashAttentionKwargs],
        ...
        # 使用 flex_attention 计算注意力
        # flex_attention 期望的输入形状: (batch, heads, seq_len, head_dim)
        # query_states, key_states, value_states 已经是正确的形状了
...
class Qwen3ForCausalLM(Qwen3PreTrainedModel, GenerationMixin):
    ...
    def __init__(self, config):
        ...
        self.attention_mask = get_full_attention_mask(2048)
    ...
    def forward_process(
        ...
        logits = self.forward(input_ids=input_ids, attention_mask=attention_mask).logits  # shape: [B, L, V]
    ...
    @torch.no_grad()
    def generate(self, prompt_ids, gen_length, num_response=1, prompt_task=None, block_size=4, temperature=1.0, ratio=None, parallel_rate=None):
        ...
        # seq: [batch, total_len]
        seq = torch.full((num_response, prompt_len + gen_length), self.config.mask_token_id, device=self.device)
        seq[:, :prompt_len] = prompt_ids.to(self.device)
        attention_mask = get_full_attention_mask(prompt_len + gen_length)
        global_step = 0
        generated_tokens = 0
        for step in range(gen_length // block_size):
            start, end = prompt_len + step * block_size, prompt_len + (step + 1) * block_size
            
            # 只要 batch 中任意一个样本在当前 block 还有 mask，就继续循环
            while seq[:, start:end].eq(self.config.mask_token_id).any():
                logits = self(seq, attention_mask=attention_mask).logits[:, :-1] # [batch, L-1, V]
                ...
                if temperature < 1e-6:
                    ...
                    # 展平 batch 维度进行采样，然后再恢复形状 [batch, L-1]
                ...
                # 构建掩码 [batch, L-1]
                valid_mask = seq[:, 1:].eq(self.config.mask_token_id)
                block_mask = torch.zeros_like(valid_mask)
                block_mask[:, start-1:end-1] = True
                masked_conf = conf.masked_fill(~(valid_mask & block_mask), -1.0)
                ...
                if parallel_rate is not None:
                    ...
                    rows_still_have_mask = (valid_mask & block_mask).any(dim=-1)  # [batch]
                ...
                seq[rows, cols + 1] = next_token[rows, cols]
        ...
        if step > 0:
            parallel_rate = (generated_tokens / num_response) / global_step
        else:
            parallel_rate = 0.0
        out_put = {
            'seq': seq,
