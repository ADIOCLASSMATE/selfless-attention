'''
This file is inspired by the code from https://github.com/Gen-Verse/MMaDA
'''
import os
import random
import re
...
from utils.utils import get_AR_attention_mask, get_config, flatten_omega_conf, load_model_tokenizer, log_grad_norm, AverageMeter, save_checkpoint, save_hf_model
...
def main():
    ...
    while global_step < config.training.max_train_steps:
        ...
        if global_step == 0 and accelerator.is_main_process:
            logger.info(f"Input ids shape: {input_ids_masked.shape}")
...
def validate(model, val_dataloader, diff_lm, accelerator, global_step):
    ...
    for step, batch in enumerate(val_dataloader):
        ...
        loss_ar = model.forward_process_ar(
