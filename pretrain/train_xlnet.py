...
from utils.utils import get_config, flatten_omega_conf, get_xlnet_mask, get_xlnet_mask_ar, load_model_tokenizer, log_grad_norm, AverageMeter, save_checkpoint, save_hf_model
...
def main():
    ...
    while global_step < config.training.max_train_steps:
        ...
        query_attention_mask, kv_attention_mask = get_xlnet_mask(v_sample=v_sample, seq_len=L, device=accelerator.device)
...
def validate(model, val_dataloader, diff_lm, accelerator, global_step):
    ...
    for step, batch in enumerate(val_dataloader):
        ...
        query_attention_mask_ar, kv_attention_mask_ar = get_xlnet_mask_ar(seq_len=L, device=accelerator.device)
        loss_ar = model(
            X0_input_ids=text_ids,
            labels=text_ids,
            query_attention_mask=query_attention_mask_ar,
            kv_attention_mask=kv_attention_mask_ar
        ).loss
