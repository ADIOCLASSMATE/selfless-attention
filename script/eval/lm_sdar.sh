...
SCRIPT_NAME="eval/sdar/eval_worker_sdar.py"

if [ -n "$VARIANT" ]; then
    CONFIG_PATH="./configs/sdar/lm_eval_sdar_${SIZE}_${VARIANT}.yaml"
    OUTPUT_NAME="sdar-${SIZE}-50BT-${VARIANT}"
else
    CONFIG_PATH="./configs/sdar/lm_eval_sdar_${SIZE}.yaml"
    OUTPUT_NAME="sdar-${SIZE}-50BT"
fi
