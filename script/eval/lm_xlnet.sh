...
if [ -n "$VARIANT" ]; then
    CONFIG_PATH="./configs/xlnet/lm_eval_xlnet_${SIZE}_${VARIANT}.yaml"
    OUTPUT_NAME="xlnet-${SIZE}-50BT-random+random-${VARIANT}"
else
    CONFIG_PATH="./configs/xlnet/lm_eval_xlnet_${SIZE}.yaml"
    OUTPUT_NAME="xlnet-${SIZE}-50BT-random+random"
fi
